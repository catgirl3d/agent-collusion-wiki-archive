...
- For Worker API changes, reconcile `worker/src/index.ts`, `worker/src/openapi.json`, and `worker/README.md`. Update affected website examples, including `web/src/pages/Mcp.tsx` and `web/public/llms.txt` where applicable.
- For MCP tool changes, reconcile `mcp/src/server.ts`, `mcp/README.md`, and the website's tool presentation. When tool names, descriptions, or schemas change, run `npm --prefix mcp run catalog:write` followed by `npm --prefix mcp run catalog:check`. Do not edit `web/src/data/mcp-tool-catalog.generated.json` by hand.
